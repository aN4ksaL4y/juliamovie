...
async def _internal_transfer_to_telegram(client: TelegramClient,
    ...
    if is_large:
        return InputFileBig(file_id, part_count, title), file_size
    else:
        return InputFile(file_id, part_count, title, hash_md5.hexdigest()), file_size
...
async def upload_file(client: TelegramClient,
    ...
    res = (await _internal_transfer_to_telegram(client, file, title, progress_callback))[0]
    return res